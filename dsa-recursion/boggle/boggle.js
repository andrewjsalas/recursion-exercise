...
function find(board, word, x, y, seen) {
  if (board[x][y] != word[0]) 
    return false;
  ...
  if (y > 0 && findFrom(board, word.slice(1), y - 1, x, seen))
    return true;

  if (y < 4 && findFrom(board, word.slice(1), y + 1, x, seen))
    return true;

  if (x > 0 && findFrom(board, word.slice(1), y, x - 1, x, seen))
    return true;

  if (x < 4 && findFrom(board, word.slice(1), y, y + 1, x, seen))
    return true;
}
...
function find(board, word) {
  for (let y = 0; y < 5; y++)
    for (let x = 0; x < 5; x++)
      if (findFrom(board, word, y, x, new Set()))
        return true;
